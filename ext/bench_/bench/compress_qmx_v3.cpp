...
/*
	ANT_ANT_COMPRESS_QMX_V3::ANT_ANT_COMPRESS_QMX_V3()
	------------------------------------
*/
ANT_compress_qmx_v3::ANT_compress_qmx_v3()
{
length_buffer = NULL;
...
/*
	ANT_ANT_COMPRESS_QMX_V3::ENCODEARRAY()
	-------------------------------
*/
void ANT_compress_qmx_v3::encodeArray(const uint32_t *source, uint64_t source_integers, uint32_t *into, uint64_t *nvalue)
{
...
*nvalue = destination - (uint8_t *)into;	// return length in bytes
	...
	/*
		The following program generates the source code for ANT_compress_qmx_v3::decodeArray()
	*/
	/*
		MAIN()
		------
		...
		if ((instance >> 4) == 0)
			...
			*/
			for (uint32_t run = 0; run < 0x10 - (instance & 0x0F); run++)
				...
				printf("\t\t\tconst __m128i byte_stream_2 = _mm_loadu_si128((__m128i *)in + %d);\n", run * 2 + 1);
	...
	*/
	...
	{
  	ANT_compress_qmx_v3 compressor;
...
#endif
/*
	ANT_ANT_COMPRESS_QMX_V3::DECODEARRAY()
	--------------------------------
	this code was generated by the method above.
*/
#include "compress_qmx_v3_decompress.cpp"
